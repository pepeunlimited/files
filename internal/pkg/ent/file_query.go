...
func (fq *FileQuery) QueryBuckets() *BucketQuery {
	query := &BucketQuery{config: fq.config}
...
func (fq *FileQuery) WithBuckets(opts ...func(*BucketQuery)) *FileQuery {
	query := &BucketQuery{config: fq.config}
...
func (fq *FileQuery) sqlAll(ctx context.Context) ([]*File, error) {
	...
	if query := fq.withBuckets; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].bucket_files; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "bucket_files" returned %v`, n.ID)
			}
			for i := range nodes {
