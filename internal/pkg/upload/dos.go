...
func NewDos() storage.Actions {
	spacesAccessKey    := misc.GetEnv(storage.SpacesAccessKey, storage.AccessKey)
	spacesSecretKey    := misc.GetEnv(storage.SpacesSecretKey, storage.SecretKey)
	doAccessToken      := misc.GetEnv(storage.DoAccessToken, storage.AccessToken)
	return dos{
